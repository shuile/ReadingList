...
* 代理模式--为另一个对象提供一个替身或占位符以控制对这个对象的访问。
  * 使用代理模式创建代码（representative）对象，让代表对象控制某对象的访问，被代理的对象可以是远程的对象、创建开销大的对象或需要安全控制的对象。
...
* 代理相关实现
  ...
  * 只能引用代理(Smart Reference Proxy)--当主题被引用时，进行额外的动作。
  * 缓存代理(Caching Proxy)--为开销大的运算结果提供暂时存储：它也允许多个客户端共享结果，以减少计算或网络延迟
  ...
  * 写入时复制代理(Copy-On-Write Proxy)--用来控制对象的赋值，方法时延迟对象的复制，直到客户端真的需要为止。这是虚拟代理的变体。
...
要点
  ...
  * 这些模式携手合作，把MVC模型的三层解耦，这样可以保值设计干净又有弹性
  * 适配器模式用来将新的模型适配城已有的视图和控制器
  * Model 2是MVC在Web上的应用
...
要点
  ...
  * 你必须相当专注才能够称为一个模式的作家：这需要时间也需要耐心，同时还必须乐意做大量的精化工作。
...
* 用途和缺点
  ...
  * 当需要不同的方式改变接口和实现时，你会大仙桥接模式很好用
  * 桥接模式的缺点是增加了复杂度

### 生成器

* 使用生成器模式封装一个产品的构构造过程，并允许按步骤构造

* 优点
  * 讲一个负载对象的创建过程封装起来
  * 允许对象通过多个步骤来创建没并且可以改变过程（这和只有一个步骤的工厂模式不同）
  * 向客户隐藏产品内部的表现
...
* 用途和缺点
  * 经常呗使用在窗口系统中，处理鼠标和键盘之类的事件
  * 并不保证请求一定会被执行；如果没有任何对象处理它的话，它可能会落到链尾端之外（这可以是有点也可以是缺点）
  * 可能不容易观察运行时的特征，有碍于除错

### 蝇量（FlyWeight Pattern）

* 如想让某个类的一个实例能用来提供许多“虚拟实例”，就是用蝇量模式

* 优点
  * 减少运行时对象实例的个数，节省内存
  * 将许多“虚拟”对象的状态集中管理
* 用途和缺点
* 当一个类有许多的实例，而这些实例能被统一方法控制的时候，我们就可以使用蝇量模式
...
* 优点
  * 将被存储的状态放在外面，不要和关键对象混在一起，这可以帮助内聚
  * 保持关键对象的数据封装
  * 提供了容易实现的恢复能力
* 用途和缺点
  * 备忘录用于存储状态
...
* 当你想要为一个对象的组合增加新的能调理，切封装并不重要时，就是用访问者模式

* 优点
  * 允许你对组合结构加入新的操作，而无须改变结构本身
  * 想要加入新的操作，相对容易
